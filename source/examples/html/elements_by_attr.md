...
The provided code showcases how to extract elements from an HTML document based
on their attributes. It specifically focuses on obtaining elements by 'class'
and 'href' attributes, employing methods that match, search from the beginning,
and search from the end of the attribute values.
...
The `base.h` header seems to contain definitions and functions crucial for this
example, while `lexbor/dom/dom.h` provides the necessary DOM manipulations for
lexbor.

### Print Collection Function

The function `print_collection_elements` is defined to handle the output of the
retrieved elements:
...
This function loops through the elements within the provided collection using
its length and utilizes the `serialize_node` function to print each element.
After processing, it ensures to clean up the collection to prevent memory leaks.
...
This string contains several `<div>` and `<a>` tags with diverse class
attributes and an `href`. The length of this HTML string is then calculated.
...
Next, a collection object is created that will hold the elements found based on
...
1. **Full Match:** Using `lxb_dom_elements_by_attr`, it searches for elements
   with the exact class `red c++ best`:
...
2. **From Beginning:** The code retrieves elements with an `href` that starts
   with `http`:
...
3. **From End:** This search targets elements with classes ending in `grep`:
...
4. **Contain:** Finally, it looks for elements where the class contains the
   substring `c++ b`:
...
Each of these searches utilizes the collection to retrieve relevant elements,
printing them as they are found.

#### Cleanup

After the searches, cleanup processes are executed to free the allocated
resources:
